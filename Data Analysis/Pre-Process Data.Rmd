...
process_file <- function(file, file_id){
  ...
  total_combinations <- length(temperatures) * length(shifts)
  
  temp = temperatures[((file_id - 1) / length(shifts)) %% length(temperatures) + 1]
  shift = shifts[((file_id - 1) %% length(shifts)) + 1]
